...
// -------------------------------------
//   Worker - Animation Fade
// -------------------------------------
/**
    * @name animation-fade.worker (used with factories/line.factory.js)
    * @desc The web worker for the app that contains functions to animate
            and fade the line through the given path in the given duration.
**/

/* (function() { *//*
    console.log("factories/line.factory/animation-fade.worker.js loaded."); */
    ...
    function _doFadeAnimation() {
        ...
        if(_counter < _updates) {
            ...
            if(!_isCancelled) { setTimeout(_doFadeAnimation, _timeout); }
            else { _endFadeAnimation(); } // end animation if it has
        }

        // check if animation is complete and
        // and trigger the callback function
        else { _endFadeAnimation(); }
    }

    // @name _endFadeAnimation
    // @desc function to call when the animation is completed or cancelled
    function _endFadeAnimation() {
    ...
    function init(options) {
        ...
        || typeof options.end !== "number" || typeof options.start !== "number") {
            var errorMessage = "animation-fade.worker.js: Cannot initialize the web worker with the given options.";
        ...
        for(var i = 1; i < _updates; i++) {
            var value = parseFloat(_easeInOutQuad(i, _start, _end - _start, _updates).toFixed(2));
            _values.push(value); // add the calculated increment for this updated to the array
        }
        ...
        /*
        // print the no.of updates and the values to confirm the fade animation curve
        console.log("---------------------------------------------------------------------------------");
        console.log("animation-fade.worker.js: No.of updates for the fade animation: " + _values.length);
